(() => {
  ...
  search.innerHTML = `
    <div class="container"><div class="major-search__inner">
      ...
      <form class="major-search__box" data-major-search-form><input type="search" data-major-search-input placeholder="예: 카페, 뷰티, 할리스, 패키지, 웹사이트, 브로셔" autocomplete="off"><button type="submit">SEARCH</button></form>
      <div class="major-search__keywords" data-major-search-keywords></div><div class="major-search__related" data-major-search-related></div>
      <div class="major-search__results" data-major-search-results hidden><div class="major-search__status"><strong data-major-search-title>Search Result</strong><span data-major-search-count>00 WORKS</span></div><div class="major-search__grid" data-major-search-grid></div></div>
    </div></div>`;
  ...
  (Array.isArray(window.NW_PORTFOLIO)?window.NW_PORTFOLIO:[]).forEach((project)=>{const key=`${project?.client||''}|${project?.title||''}`.toLowerCase();if(!project||seen.has(key))return;seen.add(key);cases.push({...project,archive:false});});
  ...
  const cardMarkup=(item)=>{const href=hrefFor(item);const isInternal=item.archive&&!archivePageFor(item);const archiveAttr=isInternal?` data-major-search-archive-id="${escapeHTML(item.id||'')}"`:'';return `<article class="major-search-card"><a href="${escapeHTML(href)}"${archiveAttr}><figure class="major-search-card__media"><img src="${escapeHTML(thumb(item))}" alt="${escapeHTML(item.title||'Project')}" loading="lazy"></figure><div class="major-search-card__info"><div><strong>${escapeHTML(item.title||'Project')}</strong><p>${escapeHTML(item.client||item.subtitle||item.scope||'')}</p></div><span>${escapeHTML(labelFor(item))}</span></div></a></article>`;};
