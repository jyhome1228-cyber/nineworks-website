(() => {
  const replaceSectionCopy = (section, title, paragraphs) => {
    ...
    if (copy && Array.isArray(paragraphs)) {
      copy.innerHTML = paragraphs.map((text) => `<p>${text}</p>`).join('');
    }
  };
  ...
  const mixDevelopCase = () => {
    ...
    rewriteDevHeading(pairs[0][0], 'A / PLANNING FRAMEWORK', '요구사항과 사용자 흐름을 먼저 정의했습니다.', '화면을 만들기 전에 고객의 질문 순서와 내부 상담 절차를 기준으로 프로젝트 요구사항을 구조화했습니다.');
    rewriteDevHeading(pairs[1][0], 'B / SYSTEM ARCHITECTURE', '웹사이트에서 관리자까지 하나의 흐름으로 연결했습니다.', '브랜드 콘텐츠, 견적 로직, 클라우드 데이터와 관리자 화면이 각자의 역할을 유지하면서 하나의 서비스 흐름으로 이어지도록 설계했습니다.');
    rewriteDevHeading(pairs[2][0], 'C / DEVELOPMENT STACK', '운영에 필요한 기술만 가볍게 구성했습니다.', '별도의 무거운 프레임워크 없이 정적 프론트엔드와 Firebase 모듈을 직접 연결해 수정과 운영이 쉬운 구조로 구축했습니다.');
    rewriteDevHeading(pairs[3][0], 'D / INFORMATION ARCHITECTURE', '메뉴 구조는 고객 행동과 내부 운영을 기준으로 정리했습니다.', '사용자용 콘텐츠 메뉴, 견적 전환 메뉴, 내부 운영 도구를 목적별로 분리해 각 기능의 역할이 명확하게 보이도록 구성했습니다.');
    rewriteDevHeading(pairs[4][0], 'E / DATA FLOW', '견적 입력값은 운영 가능한 데이터로 이어집니다.', '사용자 선택값을 상태 객체와 검증 로직을 거쳐 Firestore 문서로 저장하고, 관리자 화면에서 다시 업무 상태로 관리하도록 연결했습니다.');
    rewriteDevHeading(pairs[5][0], 'F / CODE STRUCTURE', '화면, 데이터, 관리 기능을 역할별 코드로 분리했습니다.', '핵심 기능을 상태 관리, 공개 요청 저장, 관리자 조회, 포트폴리오 업로드 단위로 나눠 유지보수 가능한 구조로 정리했습니다.');
    rewriteDevHeading(pairs[6][0], 'G / DEPLOYMENT & OPERATION', '배포 이후의 수정과 운영까지 고려했습니다.', 'Git 버전 관리, 정적 프론트엔드, Firebase 데이터 계층과 커스텀 도메인을 분리해 운영 환경을 구성했습니다.');

    sections[0].insertAdjacentElement('afterend', system);

    const planningBlock = createBlock([pairs[0]]);
    system.insertAdjacentElement('afterend', planningBlock);

    const architectureBlock = createBlock([pairs[1], pairs[2]]);
    sections[1].insertAdjacentElement('afterend', architectureBlock);

    const quoteBlock = createBlock([pairs[3], pairs[4]]);
    sections[2].insertAdjacentElement('afterend', quoteBlock);

    const adminBlock = createBlock([pairs[5]]);
    sections[3].insertAdjacentElement('afterend', adminBlock);

    const deploymentBlock = createBlock([pairs[6]]);
    sections[4].insertAdjacentElement('afterend', deploymentBlock);
  ...
  const observer = new MutationObserver(() => {
    if (mixDevelopCase()) observer.disconnect();
  });
